...
for i=1:length(speakers)
    ...
    for k=(length(test_name)-2):(length(test_name))
        ...
        Name = awgn(Name,50,'measured');
        [Id,fsd] = audioread([test_path,'\ID','\' test_id(k).name]);
        Id = awgn(Id,50,'measured');
...
figure;
confusionchart(name_true,name_pred);
figure;
confusionchart(id_true,id_pred);
...
name_stat = statsOfMeasure(name_conf, 1);
id_stat = statsOfMeasure(id_conf,1);
